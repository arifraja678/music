...
from UsuMusic import YouTube, app
from UsuMusic.core.call import Usu
from UsuMusic.misc import SUDOERS, db
...
from UsuMusic.utils.decorators.language import languageCB
from UsuMusic.utils.formatters import seconds_to_min
from UsuMusic.utils.inline.play import stream_markup, telegram_markup
from UsuMusic.utils.stream.autoclear import auto_clean
from UsuMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Usu.pause_stream(chat_id)
        ...
        await Usu.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention), disable_web_page_preview=True
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await Usu.stop_stream(chat_id)
        ...
        await Usu.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Usu.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Usu.skip_stream(chat_id, link, video=status)
            except Exception:
            ...
            try:
                await Usu.skip_stream(chat_id, file_path, video=status)
            except Exception:
                return await mystic.edit_text(_["call_7"])
            ...
            try:
                await Usu.skip_stream(chat_id, videoid, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_7"])
            button = telegram_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            try:
                await Usu.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_7"])
            if videoid == "telegram":
                button = telegram_markup(_, chat_id)
